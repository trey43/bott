...
bot.on('guildMemberAdd', member => {
   member.send(`:confetti_ball: Make Sure To Join:confetti_ball:
 **GDI Ripoff**

➭:revolving_hearts: A nice server design
➭:video_game: Talk with gamers!
➭:sunglasses: Make new friends!
➭:hugging: Newbie Friendly!
➭:relaxed: Friendly staff
➭:no_entry: No nfsw a.k.a. children friendly

:small_blue_diamond: Become a Part of this Growing server and get awesome perks!
:small_blue_diamond: Boost the server and get a special role!

Join:link:-https://discord.gg/yMSqFxvs`);
...
bot.on('guildMemberAdd', async member => {
    ...
    let welcome = await jimp.read('https://media.discordapp.net/attachments/859071718864715776/863467103529205780/1625936989866.png')

    jimp.read(member.user.displayAvatarURL({ format: 'png' })).then(avatar => {
        avatar.resize(278, 275)
        mask.resize(278, 275)
        avatar.mask(mask)
        welcome.resize(1000, 355)
        
        welcome.print(font64, 395, 125, `${member.user.username}`)
        welcome.composite(avatar, 40, 40).write('Welcome2.png')
        try {
